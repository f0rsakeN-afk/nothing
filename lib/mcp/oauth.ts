...
async function updateMcpServer({
  ...
  return prisma.mcpUserServer.update({
    where: { id, userId },
    data: values,
  });
}
...
const SLACK_DEFAULT_USER_SCOPES = [
  'canvases:read', 'canvases:write', 'channels:history', 'chat:write',
  'groups:history', 'im:history', 'mpim:history', 'search:read',
...
function stripOidcScopes(scope: string | null): string | null {
  ...
  const filtered = scope.split(/\s+/).filter((s) => !OIDC_ONLY_SCOPES.has(s)).join(' ').trim();
...
async function discoverProtectedResourceMetadata(serverUrl: string) {
  ...
  for (const candidate of metadataCandidates) {
    ...
    if (discoveredIssuer) {
      ...
      return { issuerUrl: discoveredIssuer, suggestedScope: scopeFromChallenge || scopeFromMetadata };
    }
  }
  return { issuerUrl: null, suggestedScope: bearerChallenge.scope?.trim() || null };
}
...
function resolveOAuthClientId({
  ...
  if (parsedOrigin.protocol !== 'https:' || isLocalhost) {
    throw new Error('Auto OAuth connect needs a public HTTPS app URL. In local/dev, add a provider client ID in Advanced OAuth fields.');
  }
...
async function registerDynamicOAuthClient({
  ...
  const text = await response.text();
  const parsed = safeJsonParse<{ client_id?: string; client_secret?: string; error?: string; error_description?: string }>(text);
  if (!response.ok || !parsed?.client_id) {
    const reason = parsed?.error_description || parsed?.error || text.slice(0, 200) || 'Dynamic OAuth client registration failed';
    throw new Error(reason);
  }
  return { clientId: parsed.client_id, clientSecret: parsed.client_secret ?? null };
}

export async function resolveOAuthEndpoints(
  server: Pick<McpUserServer, 'url' | 'oauthIssuerUrl' | 'oauthAuthorizationUrl' | 'oauthTokenUrl'>,
): Promise<OAuthEndpointConfig> {
  const resource = canonicalizeResourceUri(server.url);
  const isSlackMcpResource = (() => {
    try { return new URL(resource).hostname.toLowerCase() === SLACK_MCP_RESOURCE_HOST; } catch { return false; }
  })();
  ...
  if (issuerForDiscovery) {
    ...
    if (isSlackMcpResource) {
      return { ...endpoints, authorizationUrl: SLACK_AUTHORIZATION_ENDPOINT, tokenUrl: SLACK_TOKEN_ENDPOINT, resource, suggestedScope: protectedMetadata.suggestedScope || endpoints.suggestedScope || SLACK_DEFAULT_USER_SCOPES };
    }
    return { ...endpoints, resource, suggestedScope: protectedMetadata.suggestedScope || endpoints.suggestedScope || null };
  }
  ...
  try {
    ...
    return { ...endpoints, resource, suggestedScope: protectedMetadata.suggestedScope || endpoints.suggestedScope || null };
  } catch { /* Continue to explicit endpoint fallback */ }
  ...
  if (server.oauthAuthorizationUrl && server.oauthTokenUrl) {
    ...
    return { authorizationUrl, tokenUrl, resource, suggestedScope: protectedMetadata.suggestedScope || (isSlackMcpResource ? SLACK_DEFAULT_USER_SCOPES : null), registrationUrl: null, clientMetadataSupported: false };
  }

  if (isSlackMcpResource) {
    return { authorizationUrl: SLACK_AUTHORIZATION_ENDPOINT, tokenUrl: SLACK_TOKEN_ENDPOINT, resource, suggestedScope: protectedMetadata.suggestedScope || SLACK_DEFAULT_USER_SCOPES, registrationUrl: null, clientMetadataSupported: false };
  }

  throw new Error('Unable to discover OAuth authorization server. Provide issuer URL or valid MCP protected resource metadata.');
}
...
export async function buildMcpOAuthAuthorizationUrl({
  ...
  server: Pick<McpUserServer, 'id' | 'url' | 'oauthIssuerUrl' | 'oauthAuthorizationUrl' | 'oauthTokenUrl' | 'oauthClientId' | 'oauthScopes'>;
  userId: string;
  requestOrigin: string;
}) {
  const endpoints = await resolveOAuthEndpoints(server);
  const isGitHubOAuthFlow = isGitHubOAuthEndpoint(endpoints.authorizationUrl) || isGitHubOAuthEndpoint(endpoints.tokenUrl);
  ...
  const isVercelOAuthFlow = isVercelOAuthEndpoint(endpoints.authorizationUrl) || isVercelOAuthEndpoint(endpoints.tokenUrl);
  ...
  if (!clientId) {
    ...
    if (endpoints.registrationUrl) {
      try {
        const registered = await registerDynamicOAuthClient({ registrationUrl: endpoints.registrationUrl, serverId: server.id, requestOrigin });
    ...
    if (!clientId) {
      if (isGitHubOAuthFlow || isSlackOAuthFlow || isVercelOAuthFlow || isCanvaOAuthFlow) {
        const providerName = isGitHubOAuthFlow ? 'GitHub' : isSlackOAuthFlow ? 'Slack' : isVercelOAuthFlow ? 'Vercel' : 'Canva';
        throw new Error(
          isVercelOAuthFlow
            ? 'Vercel MCP OAuth requires an approved Vercel app/client ID. Auto client metadata is not supported.'
            : isCanvaOAuthFlow
              ? 'Canva MCP OAuth requires a Canva app Client ID/Secret with your app host allowed.'
              : `${providerName} OAuth requires a ${providerName} app Client ID. Add Client ID/Secret in Apps setup, then reconnect.`,
        );
      }
      try {
        clientId = resolveOAuthClientId({ serverId: server.id, configuredClientId: server.oauthClientId, requestOrigin });
      } catch (metadataError) {
        if (registrationError) throw new Error(`${registrationError.message}. Auto fallback also failed: ${metadataError instanceof Error ? metadataError.message : 'unable to resolve client metadata'}`);
        throw metadataError;
      }
    }
    if (!clientId && registrationError) throw registrationError;
  }
  ...
  const payload: OAuthStatePayload = { userId, serverId: server.id, verifier, nonce, exp };
  ...
  if (!isGitHubOAuthFlow && !isSlackOAuthFlow) params.set('resource', endpoints.resource);
...
export function verifyMcpOAuthState({
  ...
  if (payload.userId !== expectedUserId) throw new Error('OAuth state mismatch');
  if (expectedServerId && payload.serverId !== expectedServerId) throw new Error('OAuth state mismatch');
...
export async function exchangeMcpOAuthCode({
  ...
  server: Pick<McpUserServer, 'id' | 'url' | 'oauthIssuerUrl' | 'oauthAuthorizationUrl' | 'oauthTokenUrl' | 'oauthClientId' | 'oauthClientSecretEncrypted'>;
  userId: string;
  code: string;
  verifier: string;
  requestOrigin: string;
  ...
  const isGitHubOAuthFlow = isGitHubOAuthEndpoint(endpoints.authorizationUrl) || isGitHubOAuthEndpoint(endpoints.tokenUrl);
  const isSlackOAuthFlow = isSlackOAuthEndpoint(endpoints.authorizationUrl) || isSlackOAuthEndpoint(endpoints.tokenUrl);
  const clientId = resolveOAuthClientId({ serverId: server.id, configuredClientId: server.oauthClientId, requestOrigin });
  const origin = getTrustedAppOrigin(requestOrigin);
  const redirectUri = getOAuthCallbackUri(origin);
  ...
  if (!isGitHubOAuthFlow && !isSlackOAuthFlow) body.set('resource', endpoints.resource);
...
export async function resolveMcpOAuthAccessToken({
  ...
  server: Pick<McpUserServer, 'id' | 'url' | 'oauthIssuerUrl' | 'oauthAuthorizationUrl' | 'oauthTokenUrl' | 'oauthClientId' | 'oauthClientSecretEncrypted' | 'oauthAccessTokenEncrypted' | 'oauthRefreshTokenEncrypted' | 'oauthAccessTokenExpiresAt'>;
  userId: string;
}) {
  ...
  const clientId = resolveOAuthClientId({ serverId: server.id, configuredClientId: server.oauthClientId });
  const endpoints = await resolveOAuthEndpoints(server);
  const isGitHubOAuthFlow = isGitHubOAuthEndpoint(endpoints.authorizationUrl) || isGitHubOAuthEndpoint(endpoints.tokenUrl);
  const isSlackOAuthFlow = isSlackOAuthEndpoint(endpoints.authorizationUrl) || isSlackOAuthEndpoint(endpoints.tokenUrl);
  const body = new URLSearchParams({
...
// Standalone refresh function for use by auth-headers.ts
