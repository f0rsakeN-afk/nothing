...
function FormField({
    ...
    return (
      <div className="space-y-1.5">
        ...
        <Select value={String(value ?? prop.default ?? "")} onValueChange={onChange}>
          ...
          <SelectContent>
            {prop.oneOf!.map((opt) => (
              <SelectItem key={opt.const} value={opt.const}>
                {opt.title}
              </SelectItem>
            ))}
          </SelectContent>
  ...
  if (isMultiEnum) {
    ...
    const titles = prop.items?.anyOf
      ? Object.fromEntries(prop.items.anyOf.map((o) => [o.const, o.title]))
      : {};
    ...
    if (prop.type === "object") {
      ...
      if (typeof raw === "string") {
        try {
          ...
          // Ignore invalid object payloads
        }
      }
      continue;
    }
...
export function McpElicitationModal({ elicitation, onClose }: McpElicitationModalProps) {
  ...
  const urlHostname = (() => {
    try {
      return new URL(elicitation.url ?? "").hostname;
    } catch {
    ...
    >
      <DialogContent className="max-w-lg overflow-hidden p-0 border-border/60">
        ...
        <DialogFooter className="gap-2 flex-wrap border-t border-border/60 px-5 py-4 bg-muted/10">
          ...
          >
            {submitting === "cancel" && <Loader2 className="h-3 w-3 animate-spin mr-1" />}
            Cancel
          </Button>
          <Button
            size="sm"
          ...
          >
            {submitting === "decline" && <Loader2 className="h-3 w-3 animate-spin mr-1" />}
            Decline
          </Button>
          {elicitation.mode === "url" ? (
            <Button size="sm" disabled={!urlConsented || Boolean(submitting)} onClick={handleOpen}>
              {submitting === "accept" ? (
            ...
            <Button size="sm" disabled={Boolean(submitting)} onClick={() => respond("accept")}>
              {submitting === "accept" && <Loader2 className="h-3 w-3 animate-spin mr-1" />}
